(function() {
    ...
    angular.module('twicepixelsApp').factory('translationStorageProvider', translationStorageProvider);
    ...
    function translationStorageProvider($cookies, $log, LANGUAGES) {
        ...
        function get(name) {
            if (LANGUAGES.indexOf($cookies.getObject(name)) === -1) {
                $log.info('Resetting invalid cookie language "' +
                    $cookies.getObject(name) + '" to prefered language "en"');
